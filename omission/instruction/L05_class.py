...
# # 支持通过对象的属性进行模式匹配（Python 3.10）引入
...
# match order:  # Match statements require Python 3.10 or newer
...
# # 每个 case 中，先指定期望的 order 是什么对象，然后列出属性及其预期值
# # 不必列出所有属性（就像第二个 case 那样）
# # 还可以用 first = first 的方式捕获 .first 属性并使用它
# # 并最后提供兜底的方案

# # 如果介意每次输入属性名称，可以定义特殊的__match_args__类属性，
# # 以指定模式的值如何按位置映射到对象的属性
...
# # 将类属性__match_args__定义为一个字符串元组
# # 于是case中的第一个参数被映射到 __match_args__[0] = x_pos，依次类推
# # 终于，我们不用每次输入类属性的名称
...
# 每个方法都一概做一件特定的事情
# 方法的实现不应该影响其它方法，或影响程序其余部分的行为
# 避免副作用
# 通常方法不应该有状态，或受状态影响

# 属性只能由其所在实例或类的方法修改，且只能通过点运算符调用
# thing.action()  # this can mutate attributes in thing
# 对象被传递给函数时，它不应该有所改变
# action(thing)  # should not modify thing; returns new value or object
# 结合了函数式编程和面向对象编程规则的代码更容易维护
...
# # 类不是模块
# # 不该在模块足够的情况下使用类
# # 类的设计目标是将数据的访问与修改数据的方法捆绑在一起
# # 应确保类中包含的任何方法都与属性直接相关
